...
class BBP_Bayes_Net(BaseNet):
    ...
    def fit(self, x, y, samples=1):
        ...
        err = pred.ne(y.argmax(dim=1).data).sum()
        #加上acc
        acc = torch.eq(pred, y.argmax(dim=1)).sum().float().item()

        return Edkl.data, mlpdw.data, err,acc
    ...
    def eval(self, x, y, train=False):
        ...
        loss = F.cross_entropy(out, y.argmax(dim=1), reduction='sum')
        ...
        #加上acc
        acc = torch.eq(pred, y.argmax(dim=1)).sum().float().item()
        return loss.data, err, probs, acc,out
